...
`HTTP`服务的请求报文，要从`TCP`对话连接读取。`deno`实现的`TCP`对话连接数据类型是 `Conn`, 而`Conn`是基于`deno`的`Reader`、`Writer`和`Closer` 数据类型派生出来的。因此，`TCP`的对话连接是一种`Buffer`类型的数据，想要读取HTTP请求报文，就需要借助前面一篇[Buffer进阶读操作](https://github.com/chenshenhai/deno_note/blob/master/note/chapter_05/06.md)的内容，来对`HTTP`请求报文做数据读取。
